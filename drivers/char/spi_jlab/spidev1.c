...
#define FILE_NAME		"spidev1"
#define SPI0_ADDR		0xE0007000 /* spi总线控制器硬件起始地址 */
...
{	
	...
	unsigned char tx[10] = {0}, rx[10] = {0};                                                         //add by JF-YHL 2017-07-10

	//struct spi_ioc_transfer *transfer =NULL;
	struct spi_ioc_transfer *transfer = kmalloc(sizeof(struct spi_ioc_transfer),GFP_KERNEL);           //add by JF-YHL 2017-07-10
	...
	case SPI_IOC_MESSAGE: // 数据传输
		//transfer = (struct spi_ioc_transfer *)arg;
			if(copy_from_user(transfer,(unsigned long *)arg, sizeof(struct spi_ioc_transfer)))    //add by JF-YHL 2017-07-10
	   		        return -EINVAL;  
			if(copy_from_user(&tx,transfer->tx, transfer->length))                                 //add by JF-YHL 2017-07-10
					return -EINVAL;  
			if(copy_from_user(&rx,transfer->rx, transfer->length))                                  //add by JF-YHL 2017-07-10
					return -EINVAL; 
		...
		for (ix = 0; ix < transfer->length; ++ix) {
			writeregs(TX_DATA_REG, tx[ix]); // 发送数据
		}
		...
		for (ix = 0; ix < transfer->length; ++ix) { // 读数据
			rx[ix] = readregs(RX_DATA_REG);
		}
		if(copy_to_user(transfer->rx,&rx,transfer->length))             //add by JF-YHL 2017-07-10
		    return -EINVAL;
